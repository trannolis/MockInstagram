...
def search_tag():
    ...
    cursor = conn.cursor()
    query = 'SELECT pID FROM tag WHERE username = %s AND tagStatus > 0 AND (pID IN (SELECT pID FROM photo AS p NATURAL JOIN follow AS f WHERE p.allFollowers=1 AND f.follower= %s AND f.followee = p.poster AND f.followStatus = 1) OR pID IN (SELECT pID FROM sharedwith AS s NATURAL JOIN belongto AS b WHERE s.groupName = b.groupName AND s.groupCreator = b.groupCreator AND b.username = %s))'
